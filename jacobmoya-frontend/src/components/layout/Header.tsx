...
export default function Header({}: Props) {
    ...
    return (
        <header className='flex items-end justify-center text-green-500 z-10 relative pt-6'>
            <div className="flex items-center justify-start w-1/12 relative px-2">
                <h1 className=' text-black bg-green-500 p-2 w-max font-bold '>
                    JM
                </h1>
            </div>

            <nav className="nav-container text-center w-10/12 uppercase border-b-2 border-green-500 ">
                <ul className='w-full md:flex items-center md:flex-row flex-col hidden justify-evenly text-2xl '>
                    {
                        navigation.map(item=>{
                            return(
                                <ListItem 
                                    itemtext={item.text} 
                                    itemClass={ `p-1 cursor-pointer w-full md:w-2/12  border-2 border-b-0 border-green-500 -mb-0.5 ${item.isActive ? 'bg-black':''}` } 
                                    itemChildren={
            ...
            <div className="flex items-center justify-end logo-container text-right w-1/12 relative px-2">
                <h1 className=' text-black bg-green-500 p-2 px-4 w-max font-bold'>
                    X
                </h1>
            </div>
